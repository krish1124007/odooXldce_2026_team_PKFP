  ...
  if (loading && !itineraryData) {
    return (
      <div className="py-20 text-center text-slate-400">
        ...
        <p className="text-sm font-semibold">Generating Itinerary View...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white mt-1">{trip?.name || 'Trip Itinerary'}</h1>
        ...
        <div className="space-y-6">
          {sortedDates.length === 0 ? (
            <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-12 text-center space-y-3">
              ...
              <h3 className="text-base font-bold text-slate-200">No scheduled activities yet</h3>
            ...
            sortedDates.map((dateKey, dateIdx) => {
              ...
              return (
                <div key={dateKey} className="bg-slate-900/90 border border-slate-800 rounded-2xl overflow-hidden shadow-lg">
                  {/* Day Header */}
                  <div className="p-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="px-3 py-1 rounded-xl bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 font-extrabold text-xs">
                        DAY {dateIdx + 1}
                      </div>
                      <div>
                        <h3 className="text-sm font-bold text-white">{formatDate(dateKey)}</h3>
                        <p className="text-[11px] text-slate-400">{dayActivities.length} Activities Scheduled</p>
                      </div>
                    </div>
                  </div>
                    ...
                    {dayActivities.map((item) => {
                      ...
                      return (
                        <div
                          ...
                          className="p-4 bg-slate-950/60 border border-slate-800 rounded-xl hover:border-cyan-500/40 cursor-pointer transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                        >
                          <div className="flex items-start space-x-3">
                            <div className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-xs font-bold text-cyan-400 shrink-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {calendarDates.map((dateStr, idx) => {
              ...
              return (
                <div
                  key={dateStr}
                  className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-3 flex flex-col min-h-[160px]"
                >
                  <div className="flex items-center justify-between pb-2 border-b border-slate-800/80">
                    <span className="text-xs font-bold text-cyan-400">Day {idx + 1}</span>
                    <span className="text-xs font-semibold text-slate-300">{formatDateShort(dateStr)}</span>
                  </div>

                  <div className="space-y-2 flex-1">
                    {dayActs.length === 0 ? (
                      <p className="text-[11px] text-slate-600 italic py-3 text-center">No activities planned</p>
                    ) : (
                      dayActs.map((item) => {
                        const { itineraryActivity, activity, cityName } = item;
                        return (
                          <div
                            key={itineraryActivity._id}
                            onClick={() => setSelectedActivityForModal(item)}
                            className="p-2.5 rounded-lg bg-slate-900 border border-slate-800 hover:border-cyan-500/40 cursor-pointer text-xs space-y-1 transition-all"
                          >
                            <div className="flex items-center justify-between font-bold text-white">
                              <span className="truncate">{activity?.name}</span>
                              <span className="text-[10px] text-cyan-400 shrink-0 ml-1">{itineraryActivity.startTime}</span>
                            </div>
                            <div className="flex items-center justify-between text-[10px] text-slate-400">
                              <span>📍 {cityName}</span>
                              <span className="text-emerald-400 font-semibold">₹{itineraryActivity.estimatedCost}</span>
                            </div>
                          </div>
                        );
                      })
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            <div className="flex items-start justify-between border-b border-slate-800 pb-3">
              <div>
                ...
                <h3 className="text-lg font-bold text-white mt-1">{selectedActivityForModal.activity?.name}</h3>
                <p className="text-xs text-slate-400">📍 {selectedActivityForModal.cityName}</p>
              </div>
              <button onClick={() => setSelectedActivityForModal(null)} className="text-slate-400 hover:text-white">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs text-slate-300">
              <div className="p-3 bg-slate-950 rounded-xl space-y-1.5 border border-slate-800">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-white">{formatDate(selectedActivityForModal.itineraryActivity.date)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-white">{selectedActivityForModal.activity?.durationMinutes || 60} mins</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Estimated Cost:</span>
                  <span className="font-bold text-emerald-400">₹{selectedActivityForModal.itineraryActivity.estimatedCost}</span>
                </div>
              </div>
              ...
              {selectedActivityForModal.activity?.description && (
                <div>
                  ...
                  <p className="text-slate-400 leading-relaxed">{selectedActivityForModal.activity.description}</p>
                </div>
              )}
            </div>
